...
## 05/23/26
`RespParser` is done. I'm starting to work on a testing suite using `tests/test_resp_parser.cpp`. After this, I'll probably lay out some light testing for all the other current components (client/server connection, `epoll`). Then I'll probably work towards the command dispatcher.
